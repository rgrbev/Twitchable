...
public class OAuth2TokenService implements ResourceServerTokenServices {
  ...
  private Map<String, String> getMap(String path, String accessToken) {
    // ovde go dodadov ova so newPath i uspea odma :D
    String newPath = path + "?oauth_token=" + accessToken;
    this.logger.info("Getting user info from: " + newPath);
    ...
    try {
      ...
      return (Map) ((OAuth2RestOperations) ex).getForEntity(newPath , Map.class, new Object[0]).getBody();
